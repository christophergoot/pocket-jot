...
export function PagePreview({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`prose-pocket text-gray-900 dark:text-gray-100 h-full overflow-hidden ${
            highlightCover
              ? "border-gray-800 dark:border-gray-200 rounded-sm p-1"
              : ""
          }`}
          style={
            highlightCover ? { border: "2px double currentColor" } : undefined
          }
        >
          <ReactMarkdown
      ...
      {showPageNumber && (
        <div
          className="absolute bottom-1 right-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded font-medium"
          style={{ fontSize: "9px" }}
        >
          {pageNumber}
        </div>
